...
Button = (function() {
  ...
  Button.prototype.blink = function(state) {
    ...
    TweenMax.killTweensOf(this.bitmap);
    this.bitmap.alpha = 1;
    if (state) {
      return TweenMax.to(this.bitmap, 0.5, {
        alpha: .5,
        repeat: -1,
        yoyo: true
      });
...
ChooseBitmap = (function() {
  function ChooseBitmap(name, img1, img2, success, x, y) {
    this._dispatchSelection = bind(this._dispatchSelection, this);
...
ChooseText = (function() {
  function ChooseText(name, prev, text1, text2, comp, success, x, y) {
    this._dispatchEvent = bind(this._dispatchEvent, this);
...
Counter = (function() {
  function Counter(name, count, back, x, y) {
    this.doCount = bind(this.doCount, this);
...
Draggable = (function() {
  function Draggable(name, image, index, x, y) {
    this.blinkAgain = bind(this.blinkAgain, this);
    this.handleMouseDown = bind(this.handleMouseDown, this);
    this.endDragListener = bind(this.endDragListener, this);
    this.initDragListener = bind(this.initDragListener, this);
    this.onStopEvaluation = bind(this.onStopEvaluation, this);
    this.onInitEvaluation = bind(this.onInitEvaluation, this);
    this.initialize(name, image, index, x, y);
  }
  ...
  Draggable.prototype.initialize = function(name, image, index, x, y) {
    ...
    this.hitTester = new createjs.Shape();
    this.hitTester.graphics.beginFill('rgba(255,255,255,0.01)').drawRect(0, 0, image.width, image.height);
    this.inPlace = false;
    this.mouseChildren = false;
    return this.addChild(this.bitmap, this.hitTester);
  };
...
DraggableText = (function() {
  ...
  DraggableText.prototype.initialize = function(name, text, index, x, y) {
    ...
    this.hitTester = new createjs.Shape();
    this.hitTester.graphics.beginFill('rgba(255,255,255,0.01)').drawRect(-5, -5, this.text.getMeasuredWidth() + 10, this.text.getMeasuredHeight() + 10);
...
Droppable = (function() {
  ...
  Droppable.prototype.updateDrops = function() {
    var drop, drops, k, l, len1, len2, o;
    drop = arguments[0], drops = 2 <= arguments.length ? slice.call(arguments, 1) : [];
    this.drops = [];
    if (this.isArray(drop)) {
      for (k = 0, len1 = drop.length; k < len1; k++) {
        o = drop[k];
        this.drops.push(o);
      }
    } else {
      this.drops.push(drop);
      for (l = 0, len2 = drops.length; l < len2; l++) {
        o = drops[l];
        this.drops.push(o);
      }
    }
  ...
  Droppable.prototype.evaluateDrop = function(e) {
    var drop, dropped, k, len1, object, pt, ref;
    object = null;
    dropped = false;
    ref = this.drops;
...
Observer = (function() {
  ...
  Observer.prototype.notify = function(item) {
    var k, len1, ref, results, subscriber;
...
Oda = (function() {
  function Oda(imgurl, manifest, sounds) {
    ...
    this.initEvaluation = bind(this.initEvaluation, this);
    ...
    this.manifest = (function() {
      var k, len1, results;
      results = [];
      for (k = 0, len1 = manifest.length; k < len1; k++) {
        item = manifest[k];
        results.push({
          id: "" + item.id,
  ...
  Oda.prototype.insertInstructions = function(name, text, x, y, ital) {
    var frase, inst, it, k, label, len1, npos, triangle;
  ...
  Oda.prototype.createBitmap = function(name, id, x, y, position) {
    ...
    switch (position) {
      case 'tc':
        this.setReg(cont, img.width / 2, 0);
        break;
      case 'tr':
        this.setReg(cont, img.width, 0);
        break;
      case 'ml':
        this.setReg(cont, 0, img.height / 2);
        break;
      case 'mc':
        this.setReg(cont, img.width / 2, img.height / 2);
        break;
      case 'mr':
        this.setReg(cont, img.width, img.height / 2);
        break;
      case 'bl':
        this.setReg(cont, 0, img.height);
        break;
      case 'bc':
        this.setReg(cont, img.width / 2, img.height);
        break;
      case 'br':
        this.setReg(cont, img.width, img.height);
        break;
      default:
        this.setReg(cont, 0, 0);
    }
    return cont;
  ...
  Oda.prototype.createSprite = function(name, imgs, anim, x, y, position) {
    ...
    spriteImgs = (function() {
      ...
      for (k = 0, len1 = imgs.length; k < len1; k++) {
        img = imgs[k];
        results.push(this.preload.getResult(img));
      }
      return results;
    }).call(this);
    cont = new createjs.Container();
    ...
    switch (position) {
      case 'tc':
        this.setReg(cont, w / 2, 0);
        break;
      case 'tr':
        this.setReg(cont, w, 0);
        break;
      case 'ml':
        this.setReg(cont, 0, h / 2);
        break;
      case 'mc':
        this.setReg(cont, w / 2, h / 2);
        break;
      case 'mr':
        this.setReg(cont, w, h / 2);
        break;
      case 'bl':
        this.setReg(cont, 0, h);
        break;
      case 'bc':
        this.setReg(cont, w / 2, h);
        break;
      case 'br':
        this.setReg(cont, w, h);
        break;
      default:
        this.setReg(cont, 0, 0);
    }
  ...
  Oda.prototype.shuffleNoRepeat = function(a, len) {
    var copy, i, k, rand, ref, shuffle;
    copy = a.slice(0);
    shuffle = Array();
    for (i = k = 1, ref = len; 1 <= ref ? k <= ref : k >= ref; i = 1 <= ref ? ++k : --k) {
  ...
  Oda.prototype.shuffle = function(a) {
    ...
    copy = a.slice(0);
    for (i = k = ref = copy.length - 1; ref <= 0 ? k <= 0 : k >= 0; i = ref <= 0 ? ++k : --k) {
